...
public class Fight {
    ...
    public void Hit(Player human, Player enemy, int a, JLabel label,
        ...
        if (human.getHealth() <= 0 | enemy.getHealth() <= 0) {
            if (((Character) human).getWin() == 11) {
                EndFinalRound(((Character) human), action, results, dialog1, dialog2,
                        frame, label4, label5);
            } else {
    ...
    public void EndRound(Player human, Player enemy, JDialog dialog, JLabel label,
        ...
        if (human.getHealth() > 0) {
            ...
            ((Character) human).setWin();

            if (enemy instanceof ShaoKahn) {
                action.AddItems(38, 23, 8, items);
                action.AddPointsBoss(((Character) human), action.getEnemyes());
            } else {
                action.AddItems(25, 15, 5, items);
                action.AddPoints(((Character) human), action.getEnemyes());
            }
        } else {
    ...
    public void EndFinalRound(Character character, CharacterAction action,
                              ArrayList<Result> results, JDialog dialog1, JDialog dialog2, JFrame frame,
                              JLabel label1, JLabel label2) {
        String text = "Победа не на вашей стороне";
        if (character.getHealth() > 0) {
            character.setWin();
            action.AddPoints(character, action.getEnemyes());
        ...
        if (results == null) {
            ...
            for (int j = 0; j < results.size(); j++) {
                if (character.getPoints() < results.get(j).getPoints()) {
                    i++;
                }
            }
            if (i < 10) {
    ...
    public Player NewRound(Player human, JLabel label, JProgressBar pr1,
        ...
        Player enemy1 = null;
        if (((Character) human).getWin() == 6 | ((Character) human).getWin() == 11) {
